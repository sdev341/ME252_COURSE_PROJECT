...
def build_animation(points, sol, n_frames=120, fps=30):
    ...
    ax.set_aspect('equal')
    ax.set_title('Four-Bar Coupler Path Synthesis', fontsize=13, pad=10)
...
st.set_page_config(page_title="Four-Bar Linkage Synthesizer", layout="wide")
...
st.markdown("""
<style>
.stApp { background-color: #0e5f2c; color: white; }
html, body, [class*="css"] { color: white; }
section[data-testid="stSidebar"] { background-color: #0b4d24; color: white; }
.stNumberInput input, .stTextInput input { background-color: #1f7a3a; color: white; }
.stButton>button { background-color: #27ae60; color: white; border-radius: 8px; border: none; }
.stButton>button:hover { background-color: #2ecc71; color: white; }
.stDataFrame { background-color: #145a32; color: white; }
table { color: white !important; }
h1, h2, h3, h4, h5, h6 { color: white !important; }
</style>
""", unsafe_allow_html=True)

st.markdown("Enter **4 coupler-point positions** (A, B, C, D). The project can help in finding a four-bar linkage whose tracer point **I** passes through all four points given by user, verifies the Grashof condition, and animates the mechanism.")
...
user_input = st.text_area("Points (A, B, C, D)", value=default_text, height=150)
...
if run:
    with st.spinner("Synthesising (Pls Wait)…"):
    ...
    if not grashof_ok:
        st.error("No valid crank-rocker linkage found after {attempts} attempt(s).".format(attempts=attempts))
        d = grashof_details
        st.markdown(f"""
**Why it failed:**
...
Since no link can make a full 360° rotation, this linkage cannot continuously visit all four prescribed positions in a single rotation.

**Suggestions:**
- Try placing the four points closer together
- Increase the number of Grashof retries
- Try a different random seed
""")
        st.stop()

    st.success("LINKAGE FOUND....")
